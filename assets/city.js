...
(function () {
    ...
    function loadPlaces(qid) {
        ...
        var sparql = [
            ...
            '  ?item wdt:P31 ?type .',
            '  OPTIONAL { ?item wdt:P131 ?loc1 . }',
            '  OPTIONAL { ?item wdt:P276 ?loc2 . }',
            '  BIND(COALESCE(?loc1, ?loc2) AS ?city)',
            '  FILTER(',
            '    ?city = wd:' + qid + ' ||',
            '    EXISTS { ?item wdt:P131* wd:' + qid + ' } ||',
            '    EXISTS { ?item wdt:P276/wdt:P131* wd:' + qid + ' }',
            '  )',
            '  OPTIONAL {',
            '    ?article schema:about ?item ;',
            '            schema:isPartOf <https://en.wikipedia.org/> .',
    ...
    function loadOrganizations(qid) {
        ...
        var sparql = [
            ...
            '  OPTIONAL { ?item wdt:P131 ?loc2 . }',
            '  BIND(COALESCE(?loc1, ?loc2) AS ?city)',
            '  FILTER(',
            '    ?city = wd:' + qid + ' ||',
            '    EXISTS { ?item wdt:P159/wdt:P131* wd:' + qid + ' } ||',
            '    EXISTS { ?item wdt:P131* wd:' + qid + ' }',
            '  )',
            '  OPTIONAL {',
            '    ?article schema:about ?item ;',
